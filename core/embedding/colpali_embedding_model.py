...
class ColpaliEmbeddingModel(BaseEmbeddingModel):
    ...
    async def embed_for_ingestion(self, chunks: Union[Chunk, List[Chunk]]) -> List[np.ndarray]:
        ...
        if images:
            ...
            for i in range(0, len(images), self.batch_size):
                ...
                logger.debug(f"Processing image batch {i//self.batch_size + 1}/{(len(images)-1)//self.batch_size + 1} with {len(batch)} images")
                batch_start = time.time()
                batch_embeddings = await self.generate_embeddings_batch_images(batch)
                embeddings.extend(batch_embeddings)
                batch_time = time.time() - batch_start
                logger.debug(f"Image batch {i//self.batch_size + 1} processing took {batch_time:.2f}s ({batch_time/len(batch):.2f}s per image)")
        ...
        if texts:
            ...
            for i in range(0, len(texts), self.batch_size):
                ...
                logger.debug(f"Processing text batch {i//self.batch_size + 1}/{(len(texts)-1)//self.batch_size + 1} with {len(batch)} texts")
                batch_start = time.time()
                batch_embeddings = await self.generate_embeddings_batch_texts(batch)
                embeddings.extend(batch_embeddings)
                batch_time = time.time() - batch_start
                logger.debug(f"Text batch {i//self.batch_size + 1} processing took {batch_time:.2f}s ({batch_time/len(batch):.2f}s per text)")
            text_time = time.time() - text_start
            logger.info(f"All text embedding took {text_time:.2f}s ({text_time/len(texts):.2f}s per text)")
        
        total_time = time.time() - start_time
        logger.info(f"Total embed_for_ingestion took {total_time:.2f}s for {len(chunks)} chunks ({total_time/len(chunks):.2f}s per chunk)")
    ...
    async def generate_embeddings_batch_images(self, images: List[Image]) -> List[np.ndarray]:
        ...
        process_time = time.time() - process_start
        logger.debug(f"Processing {len(images)} images took {process_time:.2f}s ({process_time/len(images):.2f}s per image)")
        
        model_start = time.time()
        with torch.no_grad():
            image_embeddings = self.model(**processed_images)
        model_time = time.time() - model_start
        logger.debug(f"Model inference for {len(images)} images took {model_time:.2f}s ({model_time/len(images):.2f}s per image)")
        ...
        total_time = time.time() - start_time
        logger.debug(f"Total batch processing for {len(images)} images took {total_time:.2f}s ({total_time/len(images):.2f}s per image)")
    ...
    async def generate_embeddings_batch_texts(self, texts: List[str]) -> List[np.ndarray]:
        ...
        process_time = time.time() - process_start
        logger.debug(f"Processing {len(texts)} texts took {process_time:.2f}s ({process_time/len(texts):.2f}s per text)")
        
        model_start = time.time()
        with torch.no_grad():
            text_embeddings = self.model(**processed_texts)
        model_time = time.time() - model_start
        logger.debug(f"Model inference for {len(texts)} texts took {model_time:.2f}s ({model_time/len(texts):.2f}s per text)")
        ...
        total_time = time.time() - start_time
        logger.debug(f"Total batch processing for {len(texts)} texts took {total_time:.2f}s ({total_time/len(texts):.2f}s per text)")
